...
_.zip = function (...arr) {
  ...
  let maxLen = Math.max(..._.map(arr, el => el.length));
  let result = [];

  for (let i = 0; i < maxLen; i++) {
    result.push(_.pluck(arr, i));
  }
  return result;
};
...
_.zipStrict = function (...arr) {
  ...
  let minLen = Math.min(..._.map(arr, el => el.length));
  let result = [];
...
_.intersection = function (base, ...arrs) {
  ...
  _.each(base, function (item) {
    let isTrue = true;

    _.each(arrs, function (el) {
      if (!_.includes(el, item)) {
        isTrue = false;
      }
    });
